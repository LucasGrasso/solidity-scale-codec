...
import {AssetFilter, AssetFilterVariant, definite, wild, DefiniteParams, WildParams} from "../../../src/Xcm/v5/AssetFilter/AssetFilter.sol";
...
import {WildAsset, allOf, AllOfParams} from "../../../src/Xcm/v5/WildAsset/WildAsset.sol";
...
contract AssetFilterWrapper {
    function decode(bytes memory data) external pure returns (AssetFilter memory) {
...
contract AssetFilterTest is Test {
    ...
    function _assertRoundTrip(AssetFilter memory value, bytes memory expected) internal view {
    ...
    function testEncodeDecodeWild() public view {
        _assertRoundTrip(
            wild(WildParams({wildAsset: allOf(AllOfParams({
                id: AssetId(parent()),
