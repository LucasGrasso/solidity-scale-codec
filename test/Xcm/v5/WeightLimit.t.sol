...
import {WeightLimit, unlimited, limited, LimitedParams} from "../../../src/Xcm/v5/WeightLimit/WeightLimit.sol";
...
contract WeightLimitWrapper {
    function decode(bytes memory data) external pure returns (WeightLimit memory) {
...
contract WeightLimitTest is Test {
    ...
    function _assertRoundTrip(WeightLimit memory value, bytes memory expected) internal view {
    ...
    function testEncodeDecodeLimitedSmallWeight() public view {
        _assertRoundTrip(
            limited(LimitedParams({weight: Weight({refTime: 12, proofSize: 34})})),
            hex"013088"
        );
    }
